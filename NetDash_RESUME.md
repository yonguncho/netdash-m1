...
**사용자 확정 방향: "관제 페이지는 연결 실패된 설비와 스위치 포트 다운 또는
장비 다운 모니터링이 가장 중요하다."** 상태값을 관제에 나열하지 않는다.

- **제거**: v6.39.0에서 넣은 관제 요약의 '임계 근접 장비'·'평소와 다른 장비'·
  'SNMP 무응답 장비' 카드 전부(`renderRisk`/`#wall-risk`/`wallstats._risk_stats`).
  요약 탭 = 장애 목록만.
- **대신 임계 초과만 알람**: `metrics_poller.check_temp()` → `temp_over`/`temp_clear`
  이벤트가 기존 경로(장애목록·벨·이메일·티커)로 흐른다. 재알람 금지 +
  복귀는 임계−3°C(언저리 진동 시 폭주 방지).
- **온도는 스위치 상세보기**: `renderDetailEnv`에 최고 온도 크게(등급 색) + 센서 목록.
  **값이 없어도 숨기지 않고 사유 안내** — 사용자 확인 결과 실제 원인은
  **스위치에 툴 PC IP가 SNMP 허용 호스트로 미등록**. 그 문구를 그대로 넣고
  [SNMP] 진단 버튼으로 유도.
- 유지: 온도 임계 설정(⚙설정), 스위치 [SNMP] 진단 버튼, v6.38 포트 에러 카드.

**교훈**: 관제에 '정보성 카드'를 늘리면 정작 봐야 할 장애가 묻힌다. 상태값은
상세보기에서 보고, 관제에는 **임계를 넘은 것만** 올린다.
...
- 원인: fwStatusHtml 초기 가드가 부하 지표만 보고 조기 반환 → SSH로 모델·버전만
  수집된 방화벽은 상세가 안내문만 나와 비어 보임(v6.33.0 수명주기 이동으로 표면화).
- 수정: hasLoad 방식 — 모델·펌웨어·수명주기는 항상 표시, 부하 미수집은 사유 한 줄.
  '모델명도 안 나오는 장비'는 완전 미수집 상태(SSH/REST 수집 필요) — 안내문이 그 답.
- 상세 렌더 후 scrollIntoView(표가 길면 화면 밖에 그려져 빈 것으로 오인).
- 회귀 tests/test_fw_detail_v6331.py. Playwright 3케이스(모델만/미수집/전체) 검증.
...
- index.html 토폴로지 툴바 8버튼 이모지 → 인라인 SVG(`.ticon`, style.css).
  app.js `_TICO`(pencil/save — JS가 라벨 갱신하는 버튼), 연결 손잡이 🔗 텍스트
  → 링크 SVG 패스, 알림·확인 문구 이모지 제거(FE0F 변형 선택자 주의 — regex로).
- **선 양끝 포트 도트**(`tport-dot`): 각 끝 안쪽 27px에 도트, data-tip은 그 장비
  포트만("X 인터페이스: Gi1/0/48"), 미확인=회색. 꺾은선은 끝 구간 방향 벡터
  (aIn/bIn) 별도 계산. 합본 선 툴팁 유지.
- 회귀 `tests/test_topo_ui_v6310.py` 3건. Playwright 검증: 도트 4개·툴팁 동작.
- **미착수(사용자에게 제안만)**: 토폴로지 그리기 개선 아이디어(자동 초안·연결
  제안·자동 정렬·일괄 연결·검색 포커스) — 채택 대기.

## v6.30.0 — 관제 UI 벤더 톤 개편 (사용자: "이모지 말고 세련되게")

- wall.js `ico()`/`_ICO` 라인 SVG 아이콘(1em·currentColor) — 이모지 12곳 전면 교체.
- wall.html 헤더: SVG 로고 마크 + NETWORK OPERATIONS 서브 + LIVE 점멸 도트.
- wall.css v6.30 블록(오버라이드): 타일 좌측정렬+액센트 라인, 탭 언더라인
  세그먼트(활성=시안 밑줄), 편집/TV 버튼 알약형, 칩·재수집 버튼 통일.
- 회귀: `tests/test_wall_icons_v6300.py` — 이모지 잔재 검출 + ico/아이콘 존재.
