...
**현재 버전**: v6.10.0 (커밋 9c044fb, 릴리스 발행 완료)

## 방금 완료한 것 (v6.9.0) — 관제 재수집 재설계
사용자 요청: 재수집 버튼을 누른 설비 하나만 재확인해야 한다(대역 전체 재스캔 X).
- `core/facility.py recollect_single_host()` 신규 — 게이트웨이 스위치에 붙어
  그 IP 하나만 ping+ARP 조회, 같은 대역의 다른 설비 행은 안 건드림.
  `_status.running`(대역 전체 스캔 플래그)을 안 건드려 다른 스캔과 경합 안 함
  — v6.7.3의 409 문제 근본 원인 제거.
- `app.py facility_recollect` — start_collect_band 대신 동기 호출로 교체.
  `_facility_start_subnet`은 호출부가 사라져 완전 삭제(죽은 코드).
- `wall.js` — 무의미해진 '바쁨→중지 후 재시도' UI 제거.
- 근거: pytest 1455 PASS, 신규 15건 수정 전 실패 확인(git stash).
**상태**: 백로그 비었음 — 다음 지시 대기

## 방금 완료한 것 (v6.8.1 / v6.8.2)

### v6.8.2 — 서버 사양 엑셀 일괄 반영
사용자 요청: IP 기반으로 매칭되는 서버에 엑셀로 사양 반영.
- `core/excel_loader.py parse_server_specs()` — IP+CPU/코어/메모리/디스크 자동 매핑,
  단위 혼용('32 GB','1,024') 처리, MB·GB 두 칸 모두 수용.
- `POST /api/servers/import-specs` — **새 서버 생성 안 함**, IP 불일치는 unmatched.
- 서버 탭 "📋 사양 엑셀 반영" 버튼.

### v6.8.1 — WMI 접근거부 원인 정정
사용자 진단 출력에서 확인: "액세스 거부"는 계정 권한이 아니라 거의 항상
**UAC 원격 제한**(워크그룹 서버 + 로컬 관리자 계정 → 원격 세션에서 토큰 필터링).
→ 레지스트리 조치(LocalAccountTokenFilterPolicy=1)를 안내에 직접 포함.
부수 수정: 안내 문구가 110~150자에서 잘려 명령을 끝까지 실행 못 하던 것.
