...
> **진행 상황**: 커밋 `fc03d08` 에서 **세션 수집 계정을 장비 종류별로 분리**했다.
> (스위치 계정이 전 서버에 SSH로 시도되던 문제 — 계정 잠김 위험)
> 방화벽 화면에도 계정 입력 팝업을 붙였다 → **B-2의 절반이 해소**됐고, B-1은
> "계정을 안 쓴다고 안내하면서 실제로는 쓴다"는 **문구·동작 불일치만 남았다.**
