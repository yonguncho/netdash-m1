...
> | 항목 | 내용 |
> |---|---|
> | **B-12** | 서버 수집 후 6초 1회 갱신 → '수집중'이 사라질 때까지 감시 |
> | **B-15** | 서버실 전체 진단이 스위치 탭 버튼 대리 클릭 → 서버실 서버·방화벽만 진단, 진행바도 서버실 화면에 |
> | **B-16** | 방화벽·서버 폴러가 같은 `#room-progress`에 동시 기록 → 순차 실행 |
> | **B-17** | 화면 요약이 `Unknown` 규격을 그대로 표기 → CSV와 같은 목록으로 제외 |
> | **B-20** | 계정 관리 로드 실패 시 방화벽·PC 프로필 표가 영구 로딩 → 세 표 모두 오류+재시도 |
> | **B-21** | SMTP 인증정보 삭제 버튼 추가(저장돼 있을 때만 노출) |
> | **B-22** | 관제에 방화벽 장애 카테고리·타일 추가, `firewall_unreachable` 한글화 |
> | **B-23** | 중지 버튼이 응답을 확인하지 않아 '중지 중…'에서 굳던 것 |
> | **B-24** | 새로고침 후 진행 중 작업(수집·진단)에 진행 표시 재부착 |
> | **B-25** | 편집기 종류 판정을 서버 `topo_kind` 하나로 통일(프론트 재해석 제거) |
> | **B-26** | 페이지 셸이 토큰을 심어 주고 fetch가 헤더로 실어 보냄 — 원격 접속 실사용 가능 |
> | **B-27** | 죽은 잔재 제거(장부 대조 UI·토폴로지 모드 툴바 배선) |
> | **B-28** | 절대경로 로그 노출 — 살균 함수에 경로 규칙 추가 + reachability·facility 적용 |
>
> **MAJOR·MINOR 모두 처리 완료.** 회귀 테스트: `tests/test_backlog_minor_v630.py`
> (24건 — 수정 전 21건 실패 확인), 실동작 검증: `scripts/verify_remote_token.py`.
>
