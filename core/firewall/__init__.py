...
def collect_firewall(vendor, host, port=None, token="", username="",
    ...
    if vendor == "fortigate":
        p = port or 443
        interfaces = fortigate.get_interfaces(host, p, token, username, password, verify_ssl)
        arp = fortigate.get_arp_table(host, p, token, username, password, verify_ssl)
        return {"interfaces": interfaces, "arp": arp}
    if vendor == "paloalto":
        if not (username and password):
            raise ValueError("Palo Alto는 username/password가 필요합니다")
        return paloalto.collect(host, username, password, port or 22)
