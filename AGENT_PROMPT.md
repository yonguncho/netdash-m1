...
M7 장부 대조(Reconcile)  ✅ 완료 (Opus 리뷰 2R PASS, 커밋 bbfdf6d)
M8 장부 대조 UI 패널      ✅ 완료 (Opus 리뷰 PASS, 커밋 9cb69ba)
M9 엑셀 보고서 빌더       ✅ 완료 (Opus 리뷰 PASS, 커밋 대기)  ← 다음 여기
```

릴리스: v3.0.0 (M5~M8) 발행 완료 — GitHub Release + netdash.exe.
  v2.0.0은 기존 M4 릴리스가 점유하여 다음 메이저 v3.0.0으로 발행.
...
- M8 = reconcile UI 패널(완료). M9 = 엑셀 보고서 빌더(완료).
- M10 후보: 프로덕션본(C:\AI_WORKPLACE\NetDash) 알림 엔진(alert_engine) / 추가 벤더 파서 이식.
